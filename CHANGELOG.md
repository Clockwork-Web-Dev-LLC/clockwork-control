...
### Added
- **Fleet-wide domain ignore list for uptime probes**: Operators can configure wildcard patterns on Monitoring → Settings (e.g. `*.mystagingwebsite.com`) to skip matching sites from the probe runner, monitoring board, and issues down list so staging clones stop showing as DOWN. Matching sites drop off `/monitoring` KPIs and `/issues` nav badge counts while staying fully managed everywhere else.
...
- **WordPress.org closed / zombieware plugin detection**: Audits active plugins across fleet sites against official WordPress.org directory statuses (`plugin_directory_statuses`). Closed or removed plugins offer no updates and appear "up to date" on WordPress, but receive zero security patches. Weekly sync via `clockwork:refresh-closed-plugins` (Mondays at 03:30 UTC), matching via `ClosedPluginAuditor`, surfacing in the `/issues` dashboard with operator ignore capability (`IgnoredIssue::TYPE_PLUGIN_CLOSED`), and layout badge counting in `IssueCounter`.
...
- **Global Care Plans policy toggle & billing terminology elimination**: Added platform-level setting (`care_plans.enabled`) configurable in Settings → Fleet Policies and during Initial Setup review. When disabled, treats all fleet sites as covered for routine maintenance, nightly auto-updates, and automated security scans, while cleanly suppressing all "Care plan" / "No care plan" badges, banners, and contract cards across the entire platform. Decoupled care plan management from billing terminology across all controllers, views, and navigation tabs.

### Fixed
- **Closed-plugin detection classified every closed plugin as `not_found`**: WordPress.org serves both closed-plugin and missing-slug responses with HTTP 404; `PluginDirectoryClient` now classifies by response body (`error === 'closed'`, capturing the closure reason and `closed_date`) instead of status code. Transient fetch errors no longer overwrite any previously recorded definitive status, 404s are no longer retried, and `clockwork:refresh-closed-plugins` reports failure on a total outage.
- **Restore hardening (post-review)**: the staged panel now names the staged archive and apply verifies the archive key end-to-end (Control command and Companion both reject a mismatch); a real Discard endpoint clears restore state instead of leaving it cached for 2 hours; any apply-phase failure — including a poll timeout — now flags "maintenance left on" and the red banner renders on page load, not only inside the modal; the Restore modal prechecks SHA-256 availability and refuses archives with no hash on record; Backup Now and Restore now block each other per site; SHA-256 sidecars are written on all archive success paths, including recovery paths.
- **Runtime EOL freshness**: `runtime_eol.fetched_at` is only advanced when both endoflife.date products fetch successfully, so a partial failure no longer masks stale data; the Capacity EOL table now sorts EOL sites to the top by default.

### Documentation
- Corrected documentation errors introduced in the docs sweep: removed the nonexistent `clockwork:audit-fleet-admins` command/schedule from three pages (the fleet admin audit is computed at page load on `/security/admins`), fixed "Laravel 11" to Laravel 13 and PHP "8.3 supported" claims to match the actual `^8.4` requirement, corrected the restore presigned-URL lifetime to 24 hours, and added an "IAM requirements for restore" section (s3:GetObject on the archive prefix) to the Backup Relay guide.
- Documented PHP and WordPress runtime lifecycle tracking, Capacity dashboard integration, and endoflife.date API in Traffic + Capacity guide, API endpoints reference, and Scheduled Jobs reference.
- Documented CISA Known Exploited Vulnerabilities (KEV) daily catalog sync and active exploitation badges in Security Scans guide, API endpoints reference, and Scheduled Jobs reference.
- Documented WordPress.org closed plugin detection and REST API integration in Security Scans guide, API endpoints reference, and Scheduled Jobs reference.
...
### Changed
- Archived superseded `plans/rdap-domain-expiration.md` and `plans/accidental-noindex-watchdog.md` under `plans/archive/`.
...
### Fixed
- **Atomic self-updates & frontend asset compilation** ([#19](https://github.com/Clockwork-Web-Dev-LLC/clockwork-control/issues/19) — *Special thanks to @karenalenore!*):
...
- **High-contrast theme illegibility & white-on-white blocks** ([#20](https://github.com/Clockwork-Web-Dev-LLC/clockwork-control/issues/20) — *Special thanks to @karenalenore!*):
