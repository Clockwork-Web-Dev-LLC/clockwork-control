...
/**
 * Deploys the Companion mu-plugin over WP Engine's real per-install SSH
 * gateway. Much simpler than the SpinupWP installer
 * (App\Services\Companion\CompanionInstaller) because there's no sudo /
 * site_user privilege-drop dance to do — the SSH gateway already logs you
 * in as exactly the right user for that one install, nothing else on the
 * box is reachable or relevant.
 *
 * Docroot assumption: WP Engine's SSH gateway lands you in the install's
 * own home directory, with the WordPress root at `sites/{install}`
 * relative to it — this is WP Engine's documented convention, but treat it
 * as needing live-account confirmation before first production use, same
 * as every other structural assumption in this module (see
 * WPEngineServiceProvider's manifest description).
 */
class WPEngineCompanionInstaller implements CompanionInstallerContract
{
    ...
    public function installOrUpdate(Site $site, bool $rotateSecret = false): array
    {
        ...
        if ($install === '') {
            return ['result' => self::RESULT_FAILED, 'message' => 'Site has no wpengine_install_name.'];
        }

        if ($this->privateKey === null || $this->privateKey === '') {
            return ['result' => self::RESULT_FAILED, 'message' => 'No WP Engine SSH private key configured.'];
        }
    ...
    private function extractOnRemote(SSH2 $ssh, string $wpPath, string $base64Tarball): array
    {
        ...
        // the right user for this one install.
        $script = <<<BASH
set -euo pipefail
...
BASH;
        ...
        $sentinel = '__CLOCKWORK_WPE_EXIT__';
    ...
    /**
     * Same upsert-via-db-query + cache-flush approach as the SpinupWP
     * installer (see App\Services\Companion\CompanionInstaller::pushSecret's
     * docblock for the full reasoning) — the object-cache trap it guards
     * against isn't SpinupWP-specific, any WordPress install with a
     * persistent object cache can hit it.
     *
     * @return array{output: string, exit: int}
     */
