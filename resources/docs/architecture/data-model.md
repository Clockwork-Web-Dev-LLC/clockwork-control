...
- Backup relay: `backup_relay_enabled` (boolean, default `false`) marks whether the site is enrolled in scheduled off-host backup archiving to S3 Glacier Instant Retrieval. `backup_relay_frequency` (`daily` / `twice_weekly` / `weekly`, nullable) is a per-site cadence for unhosted/custom sites; `null` inherits the fleet setting on `/settings/backup-relay`. `backup_relay_last_archived_at` (nullable timestamp) tracks when an archive run was most recently completed. Companion/direct-to-S3 runs also store `backup_relay_last_size_bytes` and `backup_relay_last_sha256`.
...
### `site_ingest_exclusions`

Sticky denylist for host-API inventory. One row per `(hosting_provider, domain)` when an operator removes a SpinupWP or Pressable site from Clockwork while the WordPress site is still live at the host. Columns: `hosting_provider`, `provider_site_id` (the SpinupWP / Pressable id at drop time), `domain`, nullable `site_id` (the archived `sites` row; `nullOnDelete` so a later hard-delete of the site cannot recreate it), `reason`, `excluded_by`.

`clockwork:import-spinupwp` and `clockwork:import-pressable` skip a listing when the domain matches **any** exclusion (so a dropped SpinupWP site is not silently converted to Pressable) or when this provider's host site id matches (a rename at the host stays dropped). Unarchive deletes the matching rows so the next import can re-attach. Custom / WP Engine / Kinsta / Cloudways / GridPane archive does not write a row — those importers are not the resurrection path this table exists to block.
