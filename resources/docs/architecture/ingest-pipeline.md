...
- **`updateRetention`** (`PATCH /settings/ingest/retention`) configures the retention window for raw `threat_logs` (default 30 days, min 7, max 365).
- **`rebuildPartitions`** (`POST /settings/ingest/rebuild-partitions`) and `clockwork:rebuild-threat-logs-partitions` copy the keep window into monthly partitions on MySQL, swap the table, and drop the unpartitioned table so InnoDB disk space is reclaimed. A database migration automatically partitions fresh and small instances, while larger instances can trigger the background rebuild with zero downtime.
