...
|---|---|---|
| Backups | 30-day retention (DO Spaces or S3 Glacier) — SpinupWP/standalone sites; Pressable sites get provider API window | 90-day retention |
| Uptime monitoring | ✅ | ✅ |
...
| Client Report Work Logs | — | ✅ manual hours & task tracking |
...
## Client Report Work Logs (`SiteWorkLogsController`)

Care plans frequently include manual retainer hours, bespoke troubleshooting, or maintenance tasks outside of automated scans. The **Work Logs** tab on each site (`/sites/{id}/work-logs`) enables operators to log manual effort:
- **Attributes**: Date worked (`worked_on`), duration in decimal hours (`hours`), and a markdown description of the work performed (`description`).
- **Client Report Inclusion**: Stored in `site_work_logs` and automatically aggregated into monthly client reports generated by `clockwork:send-client-reports` (or downloaded as PDF/HTML).
