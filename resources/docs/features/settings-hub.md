...
* **Settings Sub-Tabs (`settings._tabs.blade.php`)**: A persistent navigation block rendered above `<x-page-header>` on every settings view. Below the `lg` breakpoint, Tier 1 is a full-width picker that lists all five pillars (`Overview`, `Agency Branding`, `Fleet Policies`, `Integrations & Alerts`, `System & Workspace`) so long labels are never clipped. At `lg` and up it is the underline tab row. Tier 2 displays a wrapping tools ribbon when the active pillar contains multiple tools, omitting ribbon clutter for single-tool pillars like Agency Branding.
