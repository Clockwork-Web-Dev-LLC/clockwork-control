...
- `schedulerStale(ageSeconds, lastRunAt)` / `schedulerRecovered()` — crontab is not spawning `php artisan schedule:run` (heartbeat older than 5 minutes) and its recovery. Fired from the next web request, not from a scheduled command. See [Runbooks → Scheduler stuck](/docs/runbooks/scheduler-stuck).
